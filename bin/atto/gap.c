/* gap.c, Atto Emacs, Public Domain, Hugh Barney, 2016, Derived from: Anthony's
 * Editor January 93 */
...
int growgap(buffer_t *bp, point_t n) {
  ...
  if (buflen == 0) {
    ...
    if (new == NULL)
      fatal(
        "%s: Failed to allocate required memory.\n"); /* Cannot edit a file
                                                         without a buffer. */
  } else {
    if (newlen < 0 || MAX_SIZE_T < (size_t)newlen) {
...
char_t *ptr(buffer_t *bp, register point_t offset) {
  ...
  return (bp->b_buf + offset +
          (bp->b_buf + offset < bp->b_gap ? 0 : bp->b_egap - bp->b_gap));
}
...
int posix_file(char *fn) {
  ...
  for (; *fn != '\0'; ++fn) {
    if (!isalnum((unsigned)*fn) && *fn != '.' && *fn != '_' && *fn != '-' &&
        *fn != '/')
      return (FALSE);
  }
  return (TRUE);
...
int save(char *fn) {
  ...
  if (fwrite(curbp->b_egap, sizeof(char), (size_t)length, fp) !=
      (size_t)length) {
...
int insert_file(char *fn, int modflag) {
  ...
  if ((size_t)(curbp->b_egap - curbp->b_gap) < sb.st_size * sizeof(char_t) &&
  ...
  curbp->b_gap += len =
    fread(curbp->b_gap, sizeof(char), (size_t)sb.st_size, fp);
